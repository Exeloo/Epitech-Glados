module SExprToInstructions(sExprToInsts, mapSArrayToVArray, sValToValueData, getLabelIndexInInst, sInstToInst) where
...
sValToValueData (SValueCall SMod) = VCall Mod
...
sValToValueData (SValueCall SAccess) = VCall Access
sValToValueData (SValueCall SModify) = VCall Modify
